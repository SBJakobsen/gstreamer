...
int stream_main (int argc, char *argv[])
{
    ...
    if (!data.pipeline || !data.source || !data.videoconvert || !data.x264enc || !data.h264parse || !data.fakesink) {
    ...
    /* Build the pipeline */
    gst_bin_add_many (GST_BIN (data.pipeline), data.source, data.videoconvert, data.x264enc, data.h264parse, data.fpssink, NULL); //fakesink removed from here as it should be null

    // source -> videoconvert -> x264enc -> 264parse > avdec_264 -> fpssink
    ...
    if (gst_element_link (data.h264parse, data.fpssink) != TRUE) {
