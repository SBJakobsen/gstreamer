...
int stream_main (int argc, char *argv[])
{
    ...
    if (!data.pipeline || !data.source || !data.mpph264enc || !data.h264parse || !data.kvssink) {
    ...
    /* Build the pipeline */
    gst_bin_add_many (GST_BIN (data.pipeline), data.source, data.mpph264enc, data.h264parse, data.kvssink, NULL); 
    ...
    if (gst_element_link (data.mpph264enc, data.h264parse) != TRUE) {
        g_print ("mpph264enc and h264parse could not be linked.\n");
        gst_object_unref (data.pipeline);
        return -1;
    }
